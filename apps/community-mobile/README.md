...
One command builds a release-signed Play `.aab` from a clean prebuild, with no EAS involved. It
bakes the same `eas.json` profile values as the iOS build of the same lane, and it refuses a bundle
whose baked API URL or version code is wrong, or that is signed with the debug key:

```sh
scripts/android-build.sh internal     # Play internal testing: staging API (https://api.civfix.dev), baked
scripts/android-build.sh production   # Play production: no baked URL, so Android resolves https://api.civfix.org
...
Signing reads the upload keystore through `scripts/android-release-patches.sh` (`CIVFIX_KEYSTORE_FILE`,
`CIVFIX_KEYSTORE_PASSWORD_FILE`, `CIVFIX_KEY_ALIAS`; the SDK from `CIVFIX_ANDROID_SDK_DIR`, else
`ANDROID_HOME`). Gradle 8.14.3 needs a JDK from 17 to 24 (the default Temurin 25 is refused), e.g.
`export JAVA_HOME=$(/usr/libexec/java_home -v 22)` on a Mac.
