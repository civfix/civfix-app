...
test("an empty object from the baked config falls back - the shipped-outage case", () => {
  ...
  assert.equal(resolveApiUrl({}, false), PROD_API_URL)
  assert.equal(resolveApiUrl({}, true), DEV_API_URL)
})

test("a configured string wins in both environments", () => {
  assert.equal(resolveApiUrl("https://staging.example.org", false), "https://staging.example.org")
  assert.equal(resolveApiUrl("https://staging.example.org", true), "https://staging.example.org")
})

test("null and undefined fall back to the environment default", () => {
  assert.equal(resolveApiUrl(null, false), PROD_API_URL)
  assert.equal(resolveApiUrl(undefined, false), PROD_API_URL)
  assert.equal(resolveApiUrl(null, true), DEV_API_URL)
  assert.equal(resolveApiUrl(undefined, true), DEV_API_URL)
})
...
test("a non-string of any shape falls back", () => {
  for (const bogus of [0, 1, true, false, [], { url: "x" }, () => "x"]) {
    assert.equal(resolveApiUrl(bogus, false), PROD_API_URL)
  }
})

test("surrounding whitespace is trimmed off a real value", () => {
  assert.equal(resolveApiUrl("  https://api.civfix.org  ", false), "https://api.civfix.org")
})
...
test("a release build never resolves to localhost, and a dev build never to production", () => {
  ...
  assert.equal(resolveApiUrl(undefined, false), "https://api.civfix.org")
  assert.equal(resolveApiUrl(undefined, true), "http://localhost:8080")
})
