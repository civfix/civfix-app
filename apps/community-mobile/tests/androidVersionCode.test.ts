...
test("android.versionCode defaults to the lowest code no Play upload has consumed", () => {
  delete process.env.CIVFIX_ANDROID_VERSION_CODE
  assert.equal(buildConfig({ config: {} }).android.versionCode, LOWEST_UNUSED_PLAY_VERSION_CODE)
})
...
test("a version code Play has already consumed, or one that is not a whole number, is refused", () => {
  for (const bad of ["5", "7.5", "seven", "-1"]) {
    process.env.CIVFIX_ANDROID_VERSION_CODE = bad
    assert.throws(() => buildConfig({ config: {} }), /CIVFIX_ANDROID_VERSION_CODE must be a whole number >= 6/)
  }
})
