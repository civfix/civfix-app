...
/**
 * mergePins: the PURE merge behind `useChat`'s derived `pins` list.
 *
 * Inputs:
 *   - `initialPins`: the first history page's `pins` array (full DTOs, pinned_at DESC, cap 25), the
 *     server-owned snapshot at load time. patchMessage keeps these copies fresh for LOCAL
...
export function mergePins(
  ...
  for (const m of liveMessages) {
    ...
    // A live/loaded copy REPLACES the initial pin (its pinnedAt is authoritative, because message_update
