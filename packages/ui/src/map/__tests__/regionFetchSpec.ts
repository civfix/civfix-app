...
// One behaviour spec for the map's region-fetch decision, run against every host copy of it (web
// `features/map/region-fetch.ts`, mobile `src/lib/mapRegion.ts`). It imports nothing at runtime so
// mobile's `node --test` can load it by relative path with type stripping alone.

type RegionFetchState = { loaded: BBox | null; requested: BBox | null }
...
export interface RegionFetchImpl {
  PAD_FACTOR: number
  EDGE_MARGIN: number
  MAX_COARSENESS: number
  SERVER_PIN_ZOOM: number
  VIEWPORT_REFERENCE_TILES: number
  impliedZoomForBBox(b: BBox): number
  serverReturnsPins(region: BBox): boolean
  padBbox(b: BBox, factor: number): BBox
  regionCovers(loaded: BBox, viewport: BBox): boolean
  decideRegionFetch(state: RegionFetchState, viewport: BBox, padFactor?: number): RegionFetchDecision
}

export const REGION_FETCH_CONSTANTS = {
  PAD_FACTOR: 0.6,
  EDGE_MARGIN: 0.12,
  MAX_COARSENESS: 3.5,
  SERVER_PIN_ZOOM: 10,
  VIEWPORT_REFERENCE_TILES: 8,
} as const

const BBOX_TOLERANCE = 1e-9

export function bboxesClose(a: BBox, b: BBox): boolean {
  return (
...
export function decisionsMatch(actual: RegionFetchDecision, expected: RegionFetchDecision): boolean {
...
export const IMPLIED_ZOOM_CASES: { name: string; bbox: BBox; zoom: number; pins: boolean }[] = [
...
export const PAD_CASES: { name: string; bbox: BBox; factor: number; padded: BBox }[] = [
...
export const COVERS_CASES: { name: string; loaded: BBox; viewport: BBox; covers: boolean }[] = [
...
export type RegionWalkResult = { keep: number; revert: number; request: number; digest: string }
 ...
 */
export function runRegionFetchWalk(impl: RegionFetchImpl, seed: number, steps: number): RegionWalkResult {
  ...
  for (let step = 0; step < steps; step++) {
    ...
    const decision = impl.decideRegionFetch(state, viewport)
