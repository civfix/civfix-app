...
import { actsAsHost, hasHostCapability, managesEvent } from "../../../data/hooks/host"
...
describe("hasHostCapability", () => {
  ...
  it("does NOT extend that fallback to a cohost, a staff member or a member", () => {
    for (const role of ["cohost", "staff", "member"] as const) {
      expect(hasHostCapability({ myCapabilities: [], myRole: role }, "manage_event"), role).toBe(false)
    }
  })
