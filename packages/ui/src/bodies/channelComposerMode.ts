/**
 * What a chat_group room's composer slot shows, given the viewer's role and the group's kind/visibility.
 * RN-free so it unit-tests directly under vitest.
 *
 ...
 * lists as a non-channel group needs no /groups/:id at all to know its members may post (the server
 * re-checks every send anyway).
